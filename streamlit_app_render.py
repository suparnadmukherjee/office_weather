"""
Sports Expert Chatbot — Streamlit UI (Render-hosted MCP server)
------------------------------------------------------------------
Identical to streamlit_app.py, except it talks to the office-weather MCP
server deployed on Render instead of a local instance — no need to run
office_weather_servers.py yourself.

Run with:       streamlit run streamlit_app_render.py
...
RENDER_MCP_URL = "https://office-weather.onrender.com/mcp"
os.environ.setdefault("MCP_SERVER_URL", RENDER_MCP_URL)

from sports_chatbot import PERSONA_LABELS, handle_turn

st.set_page_config(page_title="Sports Expert Chatbot (Render)", page_icon="🏆")
...
st.caption(f"Connected to MCP server: `{os.environ['MCP_SERVER_URL']}`")
